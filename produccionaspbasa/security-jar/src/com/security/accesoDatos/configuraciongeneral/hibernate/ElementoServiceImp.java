...
public class ElementoServiceImp extends GestorHibernate<Elemento> implements ElementoService {
	...
	public synchronized Boolean guardarElemento(Elemento elemento) {
		...
		try {
        	...
        	try{
        		...
        		logger.error("No se pudo cerrar la sesion", e);
        	}
        }
	}
	...
	public synchronized Boolean actualizarElemento(Elemento elemento) {
		...
		try {
			...
			//hacemos commit a la transaccion para que 
			//se refresque la base de datos.
			tx.commit();
			return true;
		} 
	...
	private void rollback(Transaction tx, Exception e, String mensaje){
		//si ocurre algon error intentamos hacer rollback
...
//        	}
	...
	@Override
	public Boolean actualizaTipoTrabajo(Long id , String tipoTrabajo) {
		Session session = null;
		...
		try {
        	...
        	Query query = session.createSQLQuery("UPDATE elementos ele SET tipoTrabajo='"+tipoTrabajo+"' WHERE id ="+id+"");
        	...
        	logger.error("No se pudo listar ", hibernateException);
        	System.out.println(hibernateException.getMessage() + "  " + hibernateException.getCause() );
	        return false;
        }finally{
        	try{
        		session.close();
...
//        	}
	...
	// TODO: Generar el motodo obtenerProximoContenedorDisponible
	// 
	...
	public Elemento obtenerProximoElementoDisponible(ClienteAsp clienteAsp,String codigoCliente,List<String> codigosElementoUtilizados) {
        ...
        try {
        	...
        	// crit.createCriteria("tipoElemento","te").add(Restrictions.eq("te.contenedor", true));
	        	...
	        	// se encontraba disponible o no. En caso que no, borrar automoticamente el dato de la casilla.
	        	// TODO: Descomentar este codigo y probarlo.
...
//			}
        	...
        	try{
        		...
        		logger.error("No se pudo cerrar la sesion durante el reposicionamiento", e);
        	}
        }
...
/*	@Override
	public Elemento buscarElementosParaRequerimientosPorSQL(String codigoElemento,String codigoClienteEmp,String codigoTipoRequerimiento){
		...
		try{
			...
			if(idTipoRequerimiento==2L || idTipoRequerimiento==3L)
				consulta+= "AND (elementos.estado = 'En Guarda') AND (tipoElementos.id = 2)";
			else if(idTipoRequerimiento==4L || idTipoRequerimiento==5L){
				consulta+= "AND (elementos.estado = 'En Guarda') AND (tipoElementos.id = 1)";
			}else if(idTipoRequerimiento==22L ){
				consulta+= "AND ((elementos.estado = 'En el Cliente') OR (elementos.estado = 'En Consulta'))  AND (tipoElementos.id = 2)";
			}else if(idTipoRequerimiento==14L ){
				consulta+= "AND ((elementos.estado = 'En el Cliente') OR (elementos.estado = 'En Consulta'))  AND (tipoElementos.id = 1)";
			}else if(idTipoRequerimiento==6L){
				consulta+= "AND (elementos.estado = 'En Guarda') AND (tipoElementos.id in( 9 , 10))";
			}
	...
	*/
	...
	//				"AND (clientesEmp.codigo = :codigoClienteEmp) AND ((x_operacion_elemento.estado <> 'Pendiente') or (x_operacion_elemento.elemento_id is null)) \";\n" +
			...
			else if(idTipoRequerimiento==4L || idTipoRequerimiento==5L  ||idTipoRequerimiento==6L ) {
	...
	@Override
	public Boolean revisarReferencias(Long id){
		Session session=null;

		try{
			session = getSession();

			String consulta ="SELECT elementos.id, elementos.basaReferencia, elementos.basaIdCaja, referencia.elemento_id " +
					" FROM  elementos LEFT OUTER JOIN " +
					" referencia ON elementos.id = referencia.elemento_id " +
					" WHERE (elementos.id = :codigoElemento) AND ( (elementos.basaReferencia IS NOT NULL) OR " +
					"(elementos.basaIdCaja IS NOT NULL) OR (referencia.elemento_id IS NOT NULL))";

			SQLQuery q = session.createSQLQuery(consulta);
			q.setLong("codigoElemento", id);

			if (q==null || q.list().isEmpty()) return (Boolean.FALSE);
	...
	public String revisarNumReqSQL(Long id){
		...
		try{
			...
			String consulta ="SELECT  requerimiento.numero\n" +
