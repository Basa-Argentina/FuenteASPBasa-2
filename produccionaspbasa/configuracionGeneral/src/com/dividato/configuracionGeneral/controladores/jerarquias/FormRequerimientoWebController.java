...
import com.dividato.configuracionGeneral.validadores.jerarquias.RequerimientoWebValidator;
...
@RequestMapping(
		value=
			{
				"/precargaFormularioRequerimientoWeb.html",
				"/guardarActualizarRequerimientoWeb.html",
				"/prepararActualizarRequerimientoWeb.html"
			}
		)
public class FormRequerimientoWebController {
	private RequerimientoService service;
	private RequerimientoWebValidator validator;	
	private ListaRequerimientoWebController listaRequerimientosController;
	...
	@Autowired
	public void setListaRequerimientosController(ListaRequerimientoWebController listaRequerimientosController) {
		this.listaRequerimientosController = listaRequerimientosController;
	}
	@Autowired
	public void setValidator(RequerimientoWebValidator validator) {
	 ...
	 */
	@RequestMapping(
			value="/precargaFormularioRequerimientoWeb.html",
...
//					clienteCodigo = requerimiento.getClienteEmp().getCodigo();
				...
				requerimiento.setClienteEmp(clienteEmpService.getByCodigo(clienteCodigo));
			...
			if(atributos.get("requerimientoFormulario") == null){
				if(clienteCodigo==null){
					...
					//obtengo empleado logueado
					User usuario = obtenerUser();
					Empleado empleado = empleadoService.getByID(usuario.getId());
					if(empleado != null){
						
						requerimiento.setCodigoPersonal(empleado.getCodigo());
						requerimiento.setCodigoPersonalAutorizante(empleado.getCodigo());
						requerimiento.setEmpleadoSolicitante(empleado);
						requerimiento.setClienteEmp(empleado.getClienteEmp());
						requerimiento.setClienteCodigo(empleado.getClienteEmp().getCodigo());
						requerimiento.setEmpleadoAutorizante(empleado);
						requerimiento.setListaPrecios(empleado.getClienteEmp().getListaPreciosDefecto());
					}
					...
					cli.setCodigo(clienteCodigo);
					requerimiento.setClienteEmp(clienteEmpService.getByCodigo(cli,obtenerClienteAspUser()));
					
					// requerimientoFormulario.empleadoSolicitante
					...
					if(requerimiento.getCodigoPersonal() != null && !"".equals(requerimiento.getCodigoPersonal()))
						requerimiento.setEmpleadoSolicitante(empleadoService.getByCodigo(requerimiento.getCodigoPersonal(), null, obtenerClienteAspUser()));
					
					// requerimientoFormulario.empleadoAutorizante
					...
					if(requerimiento.getCodigoPersonalAutorizante() != null && !"".equals(requerimiento.getCodigoPersonalAutorizante()))
						requerimiento.setEmpleadoAutorizante(empleadoService.getByCodigo(requerimiento.getCodigoPersonalAutorizante(), null, obtenerClienteAspUser()));
					
					// requerimientoFormulario.listaPrecios
					//obtengo la lista de precios
					requerimiento.setListaPrecios(requerimiento.getClienteEmp().getListaPreciosDefecto());
		...
		return "formularioRequerimientoWeb";
	}
	...
	@RequestMapping(
			value="/guardarActualizarRequerimientoWeb.html",
	...
	public String guardar(
		...
		//cargo las asociaciones y la accion
		...
		if(!requerimientoFormulario.isBuscarElemento() 
				&& !requerimientoFormulario.isEliminarElemento() 
			...
			if(!result.hasErrors()){	
				if(accion.equals("NUEVO")){
					...
					if(requerimientoFormulario.getSerie()==null){
						requerimientoFormulario.setSerie(serieService.listarSeriePopup("Requerimiento", "I", null, obtenerClienteAspUser(), requerimientoFormulario.getClienteEmp().getEmpresa(), true).get(0));
						if(requerimientoFormulario.getSerie()!=null)
							requerimientoFormulario.setPrefijo(new BigInteger(requerimientoFormulario.getSerie().getPrefijo()));
					}
					...
					requerimientoFormulario.setTipoTrabajo(requerimiento.getTipoTrabajo());
					setData(req, requerimientoFormulario);
		...
		//Entonces si es para buscar elementos////////////////
			...
			return "redirect:iniciarRequerimientoElemento.html?clienteCodigoString=" + requerimientoFormulario.getClienteCodigo() + "&destinoURL=prepararActualizarRequerimientoWeb.html"+idReq+"&tipoReqCodigoString="+requerimientoFormulario.getTipoRequerimientoCod();
		}
		//Entonces si es para buscar elementos sin referencia////////////////
			...
			if(requerimientoFormulario.getId()!=null)
				idReq = "&idRequerimiento="+requerimientoFormulario.getId();
			return "redirect:iniciarRequerimientoElementoSinReferencia.html?clienteCodigoString=" + requerimientoFormulario.getClienteCodigo() + "&destinoURL=prepararActualizarRequerimientoWeb.html"+idReq+"&tipoReqCodigoString="+requerimientoFormulario.getTipoRequerimientoCod();
		}
		//Entonces si es para insertar elemento directo////////////////
		else if(requerimientoFormulario.isInsertarElementoDirecto()){
			HashSet<RequerimientoReferencia> referencias = (HashSet<RequerimientoReferencia>) session.getAttribute("listaElementosRequerimientoFormulario");		
			if(referencias!=null)
				requerimientoFormulario.setListaElementos(referencias);
			...
			return preparar(String.valueOf(requerimientoFormulario.getElemento().getId()), Boolean.TRUE, session, atributos);
		}
		//Entonces es para eliminar elementos/////////////////
			...
			if(requerimientoFormulario.getListaElementos()!=null 
					&& requerimientoFormulario.getListaElementos().size()>0 
					&& requerimientoFormulario.getEliminarElementoId()!=null){
				...
				while(ite.hasNext()){
					...
					if(req.getHashCode() == requerimientoFormulario.getEliminarElementoId().intValue())
					{
						if(req.getReferencia()!=null && req.getReferencia().getId()!=null 
								&& (req.getReferencia().getDescripcionRearchivo()==null || "".equals(req.getReferencia().getDescripcionRearchivo())) 
								&& req.getReferencia().getReferenciaRearchivo()==null)
	...
	@RequestMapping(
			value="/prepararActualizarRequerimientoWeb.html",
			method=RequestMethod.POST
		)
	public String preparar(
		...
		if(requerimientoFormulario.getListaElementos()!=null && requerimientoFormulario.getListaElementos().size()>0){
			atributos.put("listaConElementos", "SI");
			requerimientoFormulario.setCantidad(requerimientoFormulario.getListaElementos().size());
		}
		else
			atributos.put("listaConElementos", "NO");
		atributos.put("requerimientoFormulario", requerimientoFormulario);
	...
	private void setData(Requerimiento o, Requerimiento d){
		if(d != null){			
			...
			o.setCantidad(d.getCantidad());
			o.setTipoTrabajo(d.getTipoTrabajo());
		}
	}
	...
	private void setAsociaciones(Requerimiento d, String accion, boolean bandera){		
		...
		{
			List<Deposito> depositos = depositoService.listarDepositoPopup(null, codigoSucursal, obtenerClienteAspUser());
			if(depositos!=null && depositos.size()>0)
				d.setDepositoDefecto(depositos.get(0));
		}
		//obtengo la lista de precios
	...
	private void setDataBasicoOperacion (Operacion operacion, Requerimiento requerimiento, TipoOperacion tipoOperacion){
		...
		operacion.setFechaAlta(requerimiento.getFechaAlta());
		//SimpleDateFormat sdf = new SimpleDateFormat("HH:mm");
		operacion.setHoraAlta(requerimiento.getHoraAlta());
		...
		operacion.setCantidadElementos(requerimiento.getCantidad());
	}
